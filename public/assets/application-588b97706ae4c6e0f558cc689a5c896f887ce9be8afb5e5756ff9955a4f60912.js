...
const endAnimationEvents = 'webkitAnimationEnd oanimationend msAnimationEnd animationend'

function hasScrolled() {
    var st = $('main').scrollTop();
    if (Math.abs(lastScrollTop - st) <= delta) return;
    //scrolled down AND past navbar height
    if (st > lastScrollTop && st > navbarHeight) {
        //hide the header
        //$('header').hide();
        $('header').addClass('animated slideOutUp')
        .one(endAnimationEvents, function(e) {
            //after animation ends
            $('header').hide().removeClass('animated slideOutUp');
        })
    }
    //scroll up
    else {
...
var heroParallax = function(elem, speed = 0.33) {
    s = $('main').scrollTop()
    imageHeight = $('.main-hero-image').outerHeight() || $('.other-hero-image').outerHeight();
    if (s < imageHeight) $(elem).css('background-position-y', `calc(50% + ${ s * speed}px)`)
}
